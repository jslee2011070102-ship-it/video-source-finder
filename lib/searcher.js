...
async function searchAllChannels(keyword, keywordEn, keywordZh, imageUrl) {
  ...
  // SerpAPI 검색용 단축 키워드 (앞 4단어) — 긴 쿼리는 site: 검색에서 결과 없음
  const shortEn = encodeURIComponent(keywordEn.split(' ').slice(0, 4).join(' '));
  const shortZh = encodeURIComponent(keywordZh.split(' ').slice(0, 3).join(' '));
  ...
  if (HAS_SERP) {
    ...
    // SerpAPI용: shortEn/shortZh (4단어 이하) 사용 → 더 많은 결과
    const shortEnStr = keywordEn.split(' ').slice(0, 4).join(' ');
    const shortZhStr = keywordZh.split(' ').slice(0, 3).join(' ');
    const jobs = [
      { ch: CHANNELS[0],  p: searchTikTok(shortEnStr) },
      { ch: CHANNELS[7],  p: searchPinterest(shortEnStr) },
      { ch: CHANNELS[3],  p: searchGoogleSite(shortZhStr, 'bilibili.com',    'Bilibili',    '📺') },
      { ch: CHANNELS[1],  p: searchGoogleSite(shortZhStr, 'douyin.com',      '抖音 Douyin', '🎵') },
      { ch: CHANNELS[2],  p: searchGoogleSite(shortEnStr, 'kwai.com',        '快手 Kwai',   '⚡') },
      { ch: CHANNELS[4],  p: searchGoogleSite(shortEnStr, 'instagram.com',   'Instagram',   '📸') },
      { ch: CHANNELS[5],  p: searchGoogleSite(shortZhStr, 'xiaohongshu.com', '小红书',      '🛍️') },
      { ch: CHANNELS[6],  p: searchGoogleSite(shortEnStr, 'lemon8-app.com',  'Lemon8',      '🍋') },
      { ch: CHANNELS[8],  p: searchGoogleSite(shortEnStr, 'amazon.com',      'Amazon',      '🛒') },
      { ch: CHANNELS[9],  p: searchGoogleSite(shortEnStr, 'aliexpress.com',  'AliExpress',  '🛍️') },
      { ch: CHANNELS[10], p: searchGoogleSite(shortZhStr, '1688.com',        '1688',        '🏭') },
      { ch: CHANNELS[11], p: searchGoogleSite(shortEnStr, 'alibaba.com',     'Alibaba',     '🌐') },
      { ch: CHANNELS[12], p: searchGoogleSite(shortZhStr, 'taobao.com',      'Taobao',      '🧡') },
      { ch: CHANNELS[13], p: searchGoogleSite(shortZhStr, 'tmall.com',       'Tmall',       '🏪') },
      { ch: CHANNELS[14], p: searchGoogleSite(shortEnStr, 'lazada.com',      'Lazada',      '🌏') },
      { ch: CHANNELS[15], p: searchGoogleSite(shortEnStr, 'shopee.com',      'Shopee',      '🟠') },
    ];
